...
resource "aws_instance" "my-ec2" {
  ...
  provisioner "remote-exec" {
    connection {
      ...
      inline = [
          "ansible-pull -U https://github.com/lolitaad1989/ansible.git -e COMPONENT=${var.COMPONENT} -e DB_PASSWORD=RoboShop@1 -e ENV=dev -e APP_VERSION=${var.APP_VERSION} -e ENV=dev robot-pull.yml"
        ]
    }
      tags = {
        Name = var.COMPONENT
